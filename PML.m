function [pmlx, pmly, pmlz] = PML(Dim,h,BC)
    ...
    X = 0:h:Dim(1);
    Y = 0:h:Dim(2); 
    Xs = h/2:h:Dim(1)+h; %staggered x grid for matx
    Ys = h/2:h:Dim(2)+h; %staggered y grid for maty
    ...
    pmlx = ((1 + 1i*sig_y(Ys))*(1./(1 + 1i*sig_x(X))));
    pmly = (1./(1 + 1i*sig_y(Y))*((1 + 1i*sig_x(Xs))));
